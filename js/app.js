...
function getMoreDetails(name, callback) {
  setTimeout(() => {
    const user = {
      name: name,
      age: 20,
      company: "KS Devware",
      email: "shamimislam19089@gmail.com",
    };
    callback(user);
  }, 1000);
}
function getAllWorkDetails(name, callback) {
  setTimeout(() => {
    const works = [name, "work1", "work2", "work3"];
    callback(works, "error");
  }, 500);
}

getAllUsers((users) => {
  getMoreDetails(users[1].name, (user) => {
    getAllWorkDetails(user.name, (works, err) => {
